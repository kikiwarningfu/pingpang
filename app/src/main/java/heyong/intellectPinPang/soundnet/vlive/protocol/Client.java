...
import heyong.intellectPinPang.soundnet.vlive.protocol.interfaces.GeneralService;
import heyong.intellectPinPang.soundnet.vlive.protocol.interfaces.PKService;
import heyong.intellectPinPang.soundnet.vlive.protocol.interfaces.ProductService;
import heyong.intellectPinPang.soundnet.vlive.protocol.interfaces.RoomService;
import heyong.intellectPinPang.soundnet.vlive.protocol.interfaces.SeatService;
import heyong.intellectPinPang.soundnet.vlive.protocol.interfaces.UserService;
...
import heyong.intellectPinPang.soundnet.vlive.protocol.model.body.PurchaseProductBody;
...
class Client {
    private static final String PRODUCT_URL = Constants.LiveServer;
...
//                .releaseBaseUrl("http://47.93.151.11:9590")
        ...
        return new Retrofit.Builder().baseUrl(Constants.LiveServer)
    ...
    //开始直播
    void productBeginLive(RequestBody body) {
        mRoomService.beginLiveBroadcast(body).enqueue(new Callback<ResponseBean<BeginLiveBroadCastBean>>() {
            @Override
            public void onResponse(Call<ResponseBean<BeginLiveBroadCastBean>> call, Response<ResponseBean<BeginLiveBroadCastBean>> response) {
                ResponseBean<BeginLiveBroadCastBean> body1 = response.body();
                XLog.e("开始直播===" + new Gson().toJson(response.body().toString()));
                for (ClientProxyListener listener : mProxyListeners) {
                    ...
                    } else if (body1.getErrorCode().equals("200")) {
                        listener.beginLiveBroadcastResponse(body1);
                    } else {
                        listener.onResponseError(Request.APP_VERSION, body1.getErrorCode(), body1.getMessage());
                    }
                }
            ...
            @Override
            public void onFailure(Call<ResponseBean<BeginLiveBroadCastBean>> call, Throwable t) {
                XLog.e("开始直播 error===" + t.getMessage());

                for (ClientProxyListener listener : mProxyListeners) {
                    listener.onResponseError(Request.PRODUCT_BEGIN_LIVE, "" + ERROR_CONNECTION, t.getMessage());
                }
    ...
    //进入直播间
    void getIntoLiveBroadcastingRoom(RequestBody body) {
        mRoomService.getIntoLiveBroadcastingRoom(body).enqueue(new Callback<ResponseBean<LiveRoomBean>>() {
            @Override
            public void onResponse(Call<ResponseBean<LiveRoomBean>> call, Response<ResponseBean<LiveRoomBean>> response) {
                ResponseBean<LiveRoomBean> body1 = response.body();
                XLog.e("进入直播间 数据 data===" + new Gson().toJson(response.body().toString()));

                for (ClientProxyListener listener : mProxyListeners) {
                    if (body1 == null) {
                        try {
                            listener.onResponseError(Request.PRODUCT_INTO_LIVE, "" + (ERROR_NULL),
                                    response.errorBody() == null ? MSG_NULL_RESPONSE : response.errorBody().string());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    } else if (body1.getErrorCode().equals("200")) {
                        listener.getIntoLiveBroadcastingRoomResponse(body1);
                    } else {
                        listener.onResponseError(Request.PRODUCT_INTO_LIVE, body1.getErrorCode(), body1.getMessage());
                    }
                }
            ...
            @Override
            public void onFailure(Call<ResponseBean<LiveRoomBean>> call, Throwable t) {
                XLog.e("进入直播间 error===" + t.getMessage());

                for (ClientProxyListener listener : mProxyListeners) {
                    listener.onResponseError(Request.PRODUCT_INTO_LIVE, "" + (ERROR_CONNECTION), t.getMessage());
                }
    ...
    //结束直播
    void endLiveBroadcast(RequestBody body) {
        mRoomService.endLiveBroadcast(body).enqueue(new Callback<ResponseBean>() {
            @Override
            public void onResponse(Call<ResponseBean> call, Response<ResponseBean> response) {
                ResponseBean<BeginLiveBroadCastBean> body1 = response.body();
                XLog.e("结束直播" + response.body().toString());

                for (ClientProxyListener listener : mProxyListeners) {
                    if (body1 == null) {
                        try {
                            listener.onResponseError(Request.PRODUCT_END_LIVE, "" + (ERROR_NULL),
                                    response.errorBody() == null ? MSG_NULL_RESPONSE : response.errorBody().string());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    } else if (body1.getErrorCode().equals("200")) {
                        listener.endLiveBroadcastResponse(body1);
                    } else {
                        listener.onResponseError(Request.PRODUCT_END_LIVE, body1.getErrorCode(), body1.getMessage());
                    }
                }
            ...
            public void onFailure(Call<ResponseBean> call, Throwable t) {
                XLog.e("结束直播 error===" + t.getMessage());

                for (ClientProxyListener listener : mProxyListeners) {
                    listener.onResponseError(Request.PRODUCT_END_LIVE, "" + (ERROR_CONNECTION), t.getMessage());
                }
    ...
    //开启云端录制
    void openCloudRecording(RequestBody body) {
        mRoomService.openCloudRecording(body).enqueue(new Callback<ResponseBean>() {
            @Override
            public void onResponse(Call<ResponseBean> call, Response<ResponseBean> response) {
                ResponseBean body1 = response.body();
                XLog.e("开启云端录制" + response.body().toString());

                for (ClientProxyListener listener : mProxyListeners) {
                    if (body1 == null) {
                        try {
                            listener.onResponseError(Request.PRODUCT_OPEN_CLOUD_RECORD, "" + (ERROR_NULL),
                                    response.errorBody() == null ? MSG_NULL_RESPONSE : response.errorBody().string());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    } else if (body1.getErrorCode().equals("200")) {
                        listener.openCloudRecording(body1);
                    } else {
                        listener.onResponseError(Request.PRODUCT_OPEN_CLOUD_RECORD, body1.getErrorCode(), body1.getMessage());
                    }
                }
            ...
            @Override
            public void onFailure(Call<ResponseBean> call, Throwable t) {
                XLog.e("开启云端录制 error===" + t.getMessage());

                for (ClientProxyListener listener : mProxyListeners) {
                    listener.onResponseError(Request.PRODUCT_OPEN_CLOUD_RECORD, "" + (ERROR_CONNECTION), t.getMessage());
                }
    ...
    //关闭云端录制
    void stopCloudRecording(RequestBody body) {
        mRoomService.stopCloudRecording(body).enqueue(new Callback<ResponseBean>() {
            @Override
            public void onResponse(Call<ResponseBean> call, Response<ResponseBean> response) {
                ResponseBean body1 = response.body();
                XLog.e("关闭云端录制" + response.body().toString());

                for (ClientProxyListener listener : mProxyListeners) {
//                    if (body1 == null) {
//                        try {
//                            listener.onResponseError(Request.PRODUCT_CLOSE_CLOUD_RECORD, ""+(ERROR_NULL),
//                                    response.errorBody() == null ? MSG_NULL_RESPONSE : response.errorBody().string());
//                        } catch (IOException e) {
//                            e.printStackTrace();
//                        }
//                    } else if (body1.getErrorCode().equals("200")) {
                    listener.stopCloudRecording(body1);
//                    } else {
//                        listener.onResponseError(Request.PRODUCT_CLOSE_CLOUD_RECORD, body1.getErrorCode(), body1.getMessage());
//                    }
                }
            ...
            @Override
            public void onFailure(Call<ResponseBean> call, Throwable t) {
                XLog.e("关闭云端录制 error===" + t.getMessage());
                for (ClientProxyListener listener : mProxyListeners) {
                    listener.stopCloudRecording(new ResponseBean());
//                    listener.onResponseError(Request.PRODUCT_CLOSE_CLOUD_RECORD, ""+(ERROR_CONNECTION), t.getMessage());
                }
    ...
    //离开直播
    void leaveLiveBroadcastingRoom(RequestBody body) {
        mRoomService.leaveLiveBroadcastingRoom(body).enqueue(new Callback<ResponseBean>() {
            @Override
            public void onResponse(Call<ResponseBean> call, Response<ResponseBean> response) {
                XLog.e("离开直播" + response.body().toString());
                ResponseBean<BeginLiveBroadCastBean> body1 = response.body();
                for (ClientProxyListener listener : mProxyListeners) {
                    if (body1 == null) {
                        try {
                            listener.onResponseError(Request.PRODUCT_LEAVE_LIVE, "" + (ERROR_NULL),
                                    response.errorBody() == null ? MSG_NULL_RESPONSE : response.errorBody().string());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    } else if (body1.getErrorCode().equals("200")) {
                        listener.leaveLiveBroadcastingRoom(body1);
                    } else {
                        listener.onResponseError(Request.PRODUCT_LEAVE_LIVE, body1.getErrorCode(), body1.getMessage());
                    }
                }
            ...
            @Override
            public void onFailure(Call<ResponseBean> call, Throwable t) {
                XLog.e("离开直播 error===" + t.getMessage());

                for (ClientProxyListener listener : mProxyListeners) {
                    listener.onResponseError(Request.PRODUCT_LEAVE_LIVE, "" + (ERROR_CONNECTION), t.getMessage());
                }
