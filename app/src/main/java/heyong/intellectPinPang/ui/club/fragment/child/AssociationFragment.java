...
import heyong.intellectPinPang.databinding.FragmentAssociationBinding;
 ...
 * 协会
 */
public class AssociationFragment extends BaseFragment<FragmentAssociationBinding, ClubViewModel> implements BaseQuickAdapter.OnItemChildClickListener, BaseQuickAdapter.OnItemClickListener, OnRefreshListener {
    ...
    private void getData(String s) {
        viewModel.getXlClubInfoList("" + pageNum, "" + pageSize,
                ""+viewModel.localTeamTitle.get(), ""+viewModel.localCity.get(), s);
    }
...
//        binding.swFresh.setRefreshHeader(mRefreshAnimHeader);
        RxBus.getDefault().toObservable(AssociationEvent.class).subscribe(tagEvents -> {
            viewModel.localTeamTitle.set(""+tagEvents.getEtClubName());
            pageNum=1;
            getData(""+textCode);
        });
        RxBus.getDefault().toObservable(AssociationEmptyEvent.class).subscribe(tagEvents -> {
