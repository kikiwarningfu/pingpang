...
public abstract class BaseMyOwnFormAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    public final static int TYPE_DEFAULT = 0;
    public Context mContext;
    // 数据源
    public List<T> mList;

    public BaseMyOwnFormAdapter(Context context){
        this(context, new ArrayList<T>());
    }

    public BaseMyOwnFormAdapter(Context context, List<T> list){
        mContext = context;
        mList = list;
    }
     ...
     * @param content 该item字符串内容
     */
    protected void setNewData(RecyclerView.ViewHolder viewHolder, int rowIndex, int columnIndex, String content,int position) {
