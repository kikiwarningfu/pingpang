...
import com.zhihu.matisse.internal.VideoBean;
...
public class SelectedItemCollection {
    ...
    public IncapableCause isAcceptable(Item item) {
        ...
        } else if (canVideoOne(item)) {
            return new IncapableCause(mContext.getString(R.string.error_video_one));
        } else if (typeConflictTime(item)) {
            return new IncapableCause(mContext.getString(R.string.error_video_time));
    ...
    private boolean typeConflictTime(Item item) {
        Gson gson = new Gson();
        VideoBean videoBean = gson.fromJson(new Gson().toJson(item), VideoBean.class);
        if (videoBean.getMimeType().equals("video/mp4")) {
            int second = videoBean.getDuration() / 1000;
            if (second > 30) {
                return true;
            } else {
                return false;
    ...
    private boolean canVideoOne(Item item) {
        Log.e(TAG, "canVideoOne: item===" + new Gson().toJson(item));
        int videoCount = 0;
        Log.e(TAG, "canVideoOne: ");
        Gson gson = new Gson();
        VideoBean videoBean = gson.fromJson(new Gson().toJson(item), VideoBean.class);
        if (videoBean.getMimeType().equals("video/mp4")) {
        ...
        if (mItems.size() > 0) {
            List<VideoBean> dotBeans = gson.fromJson(new Gson().toJson(mItems), new TypeToken<List<VideoBean>>() {
            }.getType());
            for (int m = 0; m < dotBeans.size(); m++) {
                if (dotBeans.get(m).getMimeType().equals("video/mp4")) {
                    videoCount = videoCount + 1;
                }
            }
        }
