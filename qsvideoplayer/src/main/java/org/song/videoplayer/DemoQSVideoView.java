...
public class DemoQSVideoView extends QSVideoViewHelp {
    ...
    protected LinearLayout llTopBuyPurchaseView;
    protected LinearLayout llCenterPurchaseView;
    ...
    protected void initView() {
        ...
        llTopBuyPurchaseView = findViewById(R.id.ll_top_travier_view);
        ...
        llTopBuyPurchaseView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                Log.e(TAG, "onClick: 点击了顶部的购买");
                if (listener != null) {
                    listener.click(0);
                }
            }
        });
        llCenterPurchaseView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                Log.e(TAG, "onClick: 点击了中间的购买");
                if (listener != null) {
                    listener.click(1);
                }
            }
    ...
    protected void changeUiWithStateAndMode(int status, int mode) {
        switch (status) {
            case STATE_NORMAL:
                Log.e("okhttp", "changeUiWithStateAndMode: STATE_NORMAL  根据播放状态设置ui显示/隐藏,包括控制UI ");
                showChangeViews(coverImageView, startButton);//普通状态显示封面和播放按钮

                break;
            case STATE_PREPARING:
                Log.e("okhttp", "changeUiWithStateAndMode: STATE_PREPARING  根据播放状态设置ui显示/隐藏,包括控制UI ");
                showChangeViews(loadingContainer);//初始化状态显示loading
                break;
            case STATE_PLAYING:
            case STATE_PAUSE:
            case STATE_AUTO_COMPLETE://播放中显示 播放按钮  [底部] [顶部]
                Log.e("okhttp", "changeUiWithStateAndMode: STATE_PLAYING   STATE_PAUSE STATE_AUTO_COMPLETE 根据播放状态设置ui显示/隐藏,包括控制UI ");
                showChangeViews(startButton,
                        mode >= MODE_WINDOW_FLOAT_SYS ? bottomContainer : bottomContainer,//浮窗不显示底部
...
//                showChangeViews(startButton,
//                      bottomContainer,//浮窗不显示底部
//                        topContainer);//全屏显示顶部
                break;
            case STATE_ERROR://出错显示errorContainer
                Log.e("okhttp", "changeUiWithStateAndMode: STATE_ERROR  根据播放状态设置ui显示/隐藏,包括控制UI ");
    ...
    protected void dismissControlView(int status, int mode) {
        Log.e("okhttp", "dismissControlView  自动隐藏控制UI");
...
//        bufferingContainer.setVisibility(isBuffering ? VISIBLE : INVISIBLE);
    ...
    @Override
    protected void showBuyPerchaseView(boolean isShowControlView) {
...
//        }
    ...
    public void setPurchaseView(boolean isShowView, final int times) {
        ...
        if (isShowView) {
            ...
            if (times > 0) {
                ...
                llTopBuyPurchaseView.setVisibility(GONE);
                if (startButton != null) {
...
//                        Log.e(TAG, "onTick: delta" + delta);
    ...
    protected boolean showProgressDialog(int delta, int position, int duration) {
        ...
        Log.e(TAG, "showProgressDialog: tv_duration" + Util.stringForTime(duration));
        Log.e(TAG, "showProgressDialog: tv_current" + Util.stringForTime(position + delta) + "/");
        Log.e(TAG, "showProgressDialog: tv_delta" + String.format("%s%ss",
                (delta > 0 ? "+" : ""),
                delta / 1000));
        mDialogProgressBar.setProgress((position + delta) * 100 / duration);
        Log.e(TAG, "showProgressDialog: setProgress" + (position + delta) * 100 / duration);
        Log.e(TAG, "showProgressDialog: tv_delta2=========" + delta / 1000);
        if (delta > 0) {
    ...
    public OnQsListener listener;

    public void setOnQsListener(OnQsListener listener) {
        this.listener = listener;
    }

    public interface OnQsListener {
        void click(int type);
