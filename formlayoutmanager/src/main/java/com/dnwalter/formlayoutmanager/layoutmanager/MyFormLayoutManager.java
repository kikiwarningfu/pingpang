 ...
 * @date 2019/10/14
 */
public class MyFormLayoutManager extends RecyclerView.LayoutManager {
    public static final String TAG = FormLayoutManager.class.getSimpleName();
    // 首次初始化数据完成
     ...
     */
    public MyFormLayoutManager(int columnCount, RecyclerView recyclerView) {
        this(true, columnCount, recyclerView);
        Log.e(TAG, "FormLayoutManager: columnCount===" + columnCount);
    }


    public MyFormLayoutManager(boolean isHorV, int count, RecyclerView recyclerView) {
        mIsHorV = isHorV;
        if (isHorV) {
            mColumnCount = count;
        } else {
    ...
    private Runnable mRunnable = new Runnable() {
        ...
        public void run() {
            for (int i = 0; i < getItemCount(); i++) {
                ...
                Log.e(TAG, "run: row====" + row + "column===" + column + "itemWidth===" + itemWidth + "itemHeight===" + itemHeight);
            ...
            Rect lastRect = null;
            try {
                lastRect = mItemRects.get(mItemRects.size() - 1);
            } catch (Exception e) {
            }
